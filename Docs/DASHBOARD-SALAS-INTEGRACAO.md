...
Integração iniciada. O dashboard ainda não está implementado.
Esta entrega incorpora os domínios de produção e concilia o executor CLI de
sincronização com as alterações da VPS mostradas pelo responsável.
Não houve deploy, alteração de dados no GLPI ou execução de testes PHP/navegador.
O ambiente local falhou na inicialização; as edições foram feitas pelo conector GitHub.

## Tela e regras propostas
...
Período inicial provisório: últimos 30 dias; escolha do usuário ainda pendente.
...
- Série diária de chamados, se incluída na implementação.
...
## Integração técnica a implementar
...
A conferência remota dos arquivos não substitui execução.
Executar validação PHP/JS e testes comportamentais antes de tirar o PR de rascunho.
O executor continua dependendo da implementação de lock de AssetSync; esta
entrega não demonstra garantia de concorrência entre processos.
