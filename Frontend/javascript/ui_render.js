...
window.UI = {
    ...
    const cards = lista.map(t => `
      <div class="asset-card">
        ...
        <div class="asset-card-body">
          ...
          <span class="asset-meta">Prioridade: <strong>${this._escapeHtml(prioLabel[t.prioridade] || t.prioridade || '')}</strong></span>
          ${t.abertura ? `<span class="asset-meta">Aberto em: <strong>${this._escapeHtml(t.abertura.substring(0, 10))}</strong></span>` : ''}
        </div>
  ...
  _renderCard(a, tipo = 'generic') {
    ...
    const isExpanded = canExpand && window.STATE.expandedComputerId === a.glpiId;
    const detailsState = canExpand ? window.STATE.computerDetailsById[a.glpiId] : null;
    const infoLines = [];

    if (a.patrimonio) {
      infoLines.push(`<div class="asset-info-line subtle">🏷 ${this._escapeHtml(a.patrimonio)}</div>`);
    }

    if (tipo === 'computer') {
      if (a.modelo) infoLines.push(`<div class="asset-info-line">💻 ${this._escapeHtml(a.modelo)}</div>`);
      if (a.reparticao) infoLines.push(`<div class="asset-info-line">📍 ${this._escapeHtml(a.reparticao)}</div>`);
    } else if (tipo === 'impressora') {
      if (a.modelo) infoLines.push(`<div class="asset-info-line">🖨️ ${this._escapeHtml(a.modelo)}</div>`);
      if (a.reparticao) infoLines.push(`<div class="asset-info-line">📍 ${this._escapeHtml(a.reparticao)}</div>`);
      ...
      if (a.modelo) infoLines.push(`<div class="asset-info-line">💻 ${this._escapeHtml(a.modelo)}</div>`);
      if (a.grupo) infoLines.push(`<div class="asset-info-line subtle">🏷 ${this._escapeHtml(a.grupo)}</div>`);
      if (a.reparticao) infoLines.push(`<div class="asset-info-line">📍 ${this._escapeHtml(a.reparticao)}</div>`);
    }

    return `
      <div class="asset-card ${isExpanded ? 'expanded' : ''}">
        <div class="asset-card-header">
        ...
        <div class="asset-card-footer">
          ${canExpand ? `
            <button class="btn-expand" data-computer-toggle="${a.glpiId}">
              ${isExpanded ? 'Ocultar detalhes' : 'Ver dados completos'}
            </button>
          ` : ''}
        ...
        ${canExpand ? this._renderComputerPanel(a, detailsState, isExpanded) : ''}
      </div>
    `;
  },

  _renderComputerPanel(asset, state, isExpanded) {
    if (!isExpanded) return '';

    if (!state || state.loading) {
      return `
        <div class="computer-panel">
          <div class="computer-panel-message info">Carregando dados completos do GLPI…</div>
        </div>
      `;
    }
    ...
    if (state.error && !state.data) {
      return `
        <div class="computer-panel">
          <div class="computer-panel-message error">${this._escapeHtml(state.error)}</div>
          <button class="btn-inline-secondary" data-computer-retry="${asset.glpiId}">Tentar novamente</button>
        </div>
      `;
    }
    ...
    const sections = (detail.sections || []).map(section => `
      <section class="computer-section">
        <div class="computer-section-header">
          <h4>${this._escapeHtml(section.title)}</h4>
        </div>
        <div class="computer-fields-grid">
          ${(section.fields || []).map(field => this._renderDetailField(field, draft)).join('')}
        </div>
      </section>
    `).join('');
      ...
      <div class="computer-panel">
        <div class="computer-panel-top">
          <div>
            ...
            <h3>${this._escapeHtml(detail.asset?.nome || asset.nome || 'Ativo')}</h3>
          </div>
        ...
        <form class="computer-detail-form" data-computer-form="${asset.glpiId}">
          ...
          <div class="computer-panel-actions">
            <button type="submit" class="btn-save-inline" ${state.saving ? 'disabled' : ''}>
              ${state.saving ? 'Salvando…' : 'Salvar alterações'}
            </button>
            <span class="computer-panel-hint">Campos em azul podem ser editados aqui. Relações e dropdowns continuam somente leitura por enquanto.</span>
          </div>
        </form>
      </div>
    `;
  ...
  _renderDetailField(field, draft) {
    ...
    if (field.editable) {
      if (field.inputType === 'textarea') {
        return `
          <label class="computer-field editable full">
            <span>${this._escapeHtml(field.label)}</span>
            <textarea class="computer-input computer-input-textarea" data-computer-input="${field.key}" name="${this._escapeAttr(field.key)}" rows="4">${this._escapeHtml(value)}</textarea>
          </label>
        `;
      }

      return `
        <label class="computer-field editable">
          <span>${this._escapeHtml(field.label)}</span>
          <input class="computer-input" data-computer-input="${field.key}" type="text" name="${this._escapeAttr(field.key)}" value="${this._escapeAttr(value)}" />
        </label>
      `;
    }
    ...
    return `
      <div class="computer-field readonly">
        <span>${this._escapeHtml(field.label)}</span>
        <div class="computer-readonly-value">${this._escapeHtml(field.displayValue || '—')}</div>
      </div>
    `;
  },
  ...
  renderTabs() {
    ...
    return tabs.map(t => `
      <button class="tab-btn ${window.STATE.tab === t.id ? 'active' : ''}" data-tab="${t.id}">
        ${t.label}
      </button>
    `).join('');
  },
      ...
      return `
        <div class="home-card" data-tab="${cat.tab}" onclick="window.App.go('${cat.tab}')" style="cursor: pointer;">
          ...
          <div class="home-card-info">
            <h3>${this._escapeHtml(cat.label)}</h3>
                ...
                ${cats.map(cat => {
                  ...
                  return `
                    <div style="display: flex; align-items: center; gap: 10px;">
                      ...
                      <span style="font-size: 14px; color: var(--text2); flex: 1;">${this._escapeHtml(cat.label)}</span>
  ...
  renderCarrinhos() {
    ...
    const cols = Object.entries(carrinhos).map(([nome, lista]) => {
      ...
      const items = filtrada.length
        ? filtrada.map(a => this._renderCard(a, 'apoio')).join('')
        : '<p class="empty-msg">Nenhum resultado.</p>';

      return `
        <div class="carrinho-col">
          <h3 class="carrinho-title">${this._escapeHtml(nome)} <span class="carrinho-count">${filtrada.length}/${lista.length}</span></h3>
