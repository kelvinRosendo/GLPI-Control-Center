/**
 * javascript/config/flows.config.js
 * -----------------------------------------------------------------------------
 * Configuração centralizada dos fluxos de assistência (Frontend).
 *
 * POR QUE EXISTE?
 * - Antes, os fluxos estavam hardcoded em assistance_flows.js
 * - Cada assistência tem um fluxo diferente
 * - Essa é a configuração que mais muda
 *
 * COMO FUNCIONA?
 * - Define dados visuais e ações de cada fluxo
 * - Templates de email ficam como functions
 *
 * PADRÃO UTILIZADO:
 * - Strategy Pattern: cada fluxo é uma "estratégia" diferente
 * - O Workflow não precisa saber O QUE cada fluxo faz
 *
 * NOTA SOBRE TEMPLATES:
 * - Templates de email permanecem como functions no frontend
 * - Pois dependem de dados dinâmicos do momento
 * - No backend, ficam como strings com placeholders
 */
